 ...
 * @description Accepts a pending location request. The authenticated user must be the viewer (who was requested).
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
    const { id } = params; // Get permission ID from URL params
    ...
    try {
        ...
        const permission = await SharePermission.findOne({
            _id: id,
            viewerId: currentUser._id, // Current user must be the viewer (who was requested)
        ...
        // Optionally, populate sharer details for the response
        const acceptedPermission = await permission.populate('sharerId', 'name email image');
